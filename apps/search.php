...
for($j=$startItem;$j<$startItem+$itemAmount;$j++)
{
	...
	//$category=$rall[$j][12];
	if (empty($category)) {$cat=explode(',', $rall[$j][12]); $category=$cat[0];}
	$itemid=$rall[$j][13];
	...
	//echo "<div id='mScrsht'><a href='?cnt=iinfo&category=".$category."&appn=".$item."&itemid=".$itemid."&l=".$lg."'>";
	...
	// End Main Screenshot
	echo "</td><td>";
	echo "<div id='appWinName'> <a href='?cnt=iinfo&category=".$category."&appn=".$item."&itemid=".$itemid."&l=".$lg."'><h2>".$rall[$j][0]."</h2></a></div><br/>";
	...
	// Begin Shot Description
	if($lg=='en') {$shd = $rall[$j][8]; $nl=300;} else {$shd = $rall[$j][8]; $nl=300;}
	$sd = substr($shd,0,$nl);
	echo $sd."...";
	// End Shot Description
