...
public  function pageNmbrPrnt($pp,$cntlink,$lg)
	{
//		echo $pp;
//		$naop=10;
		if ($pp==0) $pp=1;
		...
		if($this->pn!=1) echo "<a href='".$htmRP."?".$cntlink."&pn=".($this->pn-1)."&l=".$lg."'> < </a>";
			
		for($p=$sp; $p<=$lp; $p++) 
		{	
			if($p==$this->pn) echo "<a href='".$htmRP."?".$cntlink."&pn=".$p."&l=".$lg."'> &nbsp <b>".$p."</b> &nbsp</a>";	
			else echo "<a href='".$htmRP."?".$cntlink."&pn=".$p."&l=".$lg."'> &nbsp ".$p." &nbsp</a>";	
		}
		
		if($this->pn!=$lp) echo "<a href='".$htmRP."?".$cntlink."&pn=".($this->pn+1)."&l=".$lg."'> > </a>";
