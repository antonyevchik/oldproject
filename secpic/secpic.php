...
session_start();
 ...
 {
   $color = imagecolorallocatealpha($src,rand(0,255),rand(0,255),rand(0,255),80); 
 ...
 {
   $color = imagecolorallocatealpha($src,$colors[rand(0,sizeof($colors)-1)],$colors[rand(0,sizeof($colors)-1)],$colors[rand(0,sizeof($colors)-1)],rand(50,70)); 
   $font = $path_fonts.$fonts[rand(0,sizeof($fonts)-1)];
   $letter = $letters[rand(0,sizeof($letters)-1)];
   $size = rand($font_size*2.1-2,$font_size*2.1+2);
   $x = ($i+1)*$font_size + rand(3,5);
   $y = (($height*2)/3) + rand(0,5);
   $cod[] = $letter;   
   imagettftext($src,$size,rand(-20,30),$x,$y,$color,$font,$letter);
 }
